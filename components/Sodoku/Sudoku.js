...
const getConflicts = (currentBoard) => {
    ...
    const findConflicts = (arr, getKey) => {
        const seen = {};
        arr.forEach((num, i) => {
            if (num !== EMPTY_CELL) {
        ...
        for (const num in seen) {
            if (seen[num].length > 1) {
                seen[num].forEach(i => newConflicts.add(getKey(i)));
            }
        }
    };
    ...
    // Check rows
    for (let i = 0; i < 9; i++) {
        findConflicts(currentBoard[i], (col) => `${i}-${col}`);
    }

    // Check columns
    for (let i = 0; i < 9; i++) {
        findConflicts(currentBoard.map(r => r[i]), (row) => `${row}-${i}`);
    }
    ...
    for (let br = 0; br < 3; br++) {
        for (let bc = 0; bc < 3; bc++) {
            ...
            for (let r = 0; r < 3; r++) {
                for (let c = 0; c < 3; c++) {
                    box.push(currentBoard[br * 3 + r][bc * 3 + c]);
                }
            }
            findConflicts(box, (i) => `${br * 3 + Math.floor(i / 3)}-${bc * 3 + (i % 3)}`);
        }
    }
        ...
        <View style={gameOverStyles.modalOverlay}>
            <View style={gameOverStyles.modalContent}>
                {isLoading ? (
                    ...
                    <>
                        <View style={gameOverStyles.heartContainer}>
                            <View style={gameOverStyles.heart}>
                                <Text style={gameOverStyles.heartIcon}>❤️</Text>
                            </View>
                            <View style={gameOverStyles.plusTwoContainer}>
                                <Text style={gameOverStyles.plusTwo}>+2</Text>
                            </View>
                        </View>
                        <Text style={gameOverStyles.title}>Game Over</Text>
                        <Text style={gameOverStyles.subtitle}>
                            You've reached the mistake limit. Watch a video for a second chance!
                        </Text>
                        <TouchableOpacity style={gameOverStyles.secondChanceButton} onPress={onSecondChance}>
                            <Ionicons name="film" size={24} color="white" style={{ marginRight: 10 }} />
                            <Text style={gameOverStyles.secondChanceText}>Second chance</Text>
...
export const Sudoku = ({ navigation, route }) => {
    ...
    // Game over check
    useEffect(() => {
        if (mistakes >= mistakeLimit && isGameActive) {
            setIsGameActive(false);
            setIsGameOver(true);
        }
    }, [mistakes, mistakeLimit, isGameActive]);

    // Ad setup
    useEffect(() => {
        const hintErr = hintRewarded.addAdEventListener(AdEventType.ERROR, (err) => {
            if (isHintAdLoading) {
                console.log(err);
                setIsHintAdLoading(false);
            }
        });
        const hintLoad = hintRewarded.addAdEventListener(RewardedAdEventType.LOADED, () => {
            setHintAdLoaded(true);
            if (isHintAdLoading && !userCanceledHintAd.current) {
                setIsHintAdLoading(false);
                hintRewarded.show();
            }
        });
        const hintEarn = hintRewarded.addAdEventListener(RewardedAdEventType.EARNED_REWARD, () => {
            setHintsLeft(p => p + 2);
        });
        const hintClose = hintRewarded.addAdEventListener(AdEventType.CLOSED, () => {
            setHintAdLoaded(false);
            hintRewarded.load();
        });
        hintRewarded.load();

        const goErr = gameOverRewarded.addAdEventListener(AdEventType.ERROR, () => {
            if (isGameOverAdLoading) {
                Alert.alert("Second Chance Not Available");
                setIsGameOverAdLoading(false);
            }
        });
        const goLoad = gameOverRewarded.addAdEventListener(RewardedAdEventType.LOADED, () => {
            setGameOverAdLoaded(true);
            if (isGameOverAdLoading) {
                setIsGameOverAdLoading(false);
                gameOverRewarded.show();
            }
        });
        const goEarn = gameOverRewarded.addAdEventListener(RewardedAdEventType.EARNED_REWARD, () => {
            setMistakeLimit(p => p + 2);
            setIsGameOver(false);
            setIsGameActive(true);
        });
        const goClose = gameOverRewarded.addAdEventListener(AdEventType.CLOSED, () => {
            setGameOverAdLoaded(false);
            gameOverRewarded.load();
        });
        gameOverRewarded.load();
    ...
    const startNewGame = () => {
        ...
        setBoard(puzzleData.board.map(r => [...r]));
        setInitialBoard(puzzleData.board.map(r => [...r]));
        setSolution(puzzleData.solution.map(r => [...r]));
        setSelectedCell(null);
        setConflicts(getConflicts(puzzleData.board));
        setMoveHistory([]);
        setMistakes(0);
    ...
    const makeMove = (row, col, newValue) => {
        ...
        setMoveHistory(p => [...p, { row, col, oldValue, newValue }]);
        const newBoard = board.map(r => [...r]);
        newBoard[row][col] = newValue;
        
        if (newValue !== EMPTY_CELL && solution[row][col] !== newValue) {
            setMistakes(p => p + 1);
            setConflicts(new Set([...getConflicts(newBoard), `${row}-${col}`]));
        } else {
    ...
    const handleEmojiPress = (emoji) => {
        ...
        if (initialBoard[row][col] === EMPTY_CELL) {
            makeMove(row, col, EMOJIS.indexOf(emoji) + 1);
        }
    };
    ...
    useEffect(() => {
        if (board.length > 0 && isGameActive) {
            const isBoardFull = !board.flat().includes(EMPTY_CELL);
            if (isBoardFull && conflicts.size === 0) {
                setIsGameActive(false);
                setIsLevelComplete(true);
            }
        }
    }, [board, conflicts, isGameActive]);

    const getCellStyles = (row, col) => {
        const styles = [style.cell];
        
        if ((col + 1) % 3 === 0 && col !== 8) styles.push(style.subgridBorderRight);
        if ((row + 1) % 3 === 0 && row !== 8) styles.push(style.subgridBorderBottom);
        
        const isUserFilled = initialBoard.length > 0 && initialBoard[row][col] === EMPTY_CELL && board[row][col] !== EMPTY_CELL;
        ...
        if (selectedCell) {
            ...
            if (row === selRow || col === selCol || 
                ...
                styles.push(style.highlightedCell);
            }
        }
        
        if (isUserFilled) {
            if (isInConflict) {
                styles.push(style.conflictCell);
            } else {
                styles.push(style.correctCell);
            }
        }
        
        if (selectedCell && row === selectedCell.row && col === selectedCell.col) {
            styles.push(style.selectedCell);
        }
    ...
    const handleHint = () => {
        ...
        if (hintsLeft > 0) {
            ...
            if (board[row][col] === EMPTY_CELL) {
                makeMove(row, col, solution[row][col]);
                setHintsLeft(p => p - 1);
            }
        } else {
            userCanceledHintAd.current = false;
            if (hintAdLoaded) {
                hintRewarded.show();
            } else {
                setIsHintAdLoading(true);
            }
        }
    };

    const handleCancelHintAd = () => {
        userCanceledHintAd.current = true;
        setIsHintAdLoading(false);
    };
    ...
    return (
        <View style={style.container}>
                ...
                {board.length > 0 && board.map((row, rIndex) => (
                    <View key={rIndex} style={style.row}>
                        {row.map((num, cIndex) => (
                            <TouchableOpacity 
                                key={cIndex} 
                                style={getCellStyles(rIndex, cIndex)} 
            ...
            <ControlPanel 
                ...
                onHint={handleHint} 
                onErase={handleErase} 
                canUndo={moveHistory.length > 0 && isGameActive} 
                hintsLeft={hintsLeft} 
